...
def parse_wiki_index(url: str):
    ...
    index = pd.read_html(StringIO(html.text))[6]
    index["Ticker"] = index["Ticker"].str.replace(".", "-", regex=False) 
    index["Ticker"] += ".L"
    return index
...
index = index.rename(
    columns={
        "Ticker": "symbol",
        "Company": "name",
        "FTSE industry classification benchmark sector[39]": "sector",
    }
)
